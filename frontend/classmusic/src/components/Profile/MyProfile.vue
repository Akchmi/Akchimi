  ...
  methods: {
    ...
    deleteLikeTeacher(likeId) {
      ...
      apiDeleteLIkeTeacher(data)
        .then(() => {
          this.liketeachers = this.liketeachers.filter(
            (teacher) => teacher.likeId !== likeId
          );
        })
        .catch((error) => {
          console.log(error);
    ...
    async changePassword() {
      ...
      if (response.success) {
        ...
        alert(
          "비밀번호 변경 중 오류가 발생했습니다. 현재 비밀번호를 확인하세요."
        );
        this.currentPassword = "";
        this.newPassword = "";
      }
