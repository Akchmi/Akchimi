          ...
          <ul class="navbar__menu">
            <li  v-if="isLoggedIn">
              <div class="alarm-container">
                ...
                />
                <div v-if="alarmCount" class="notification-count" @click.stop="toggleDropdown">{{ alarmCount }}</div>
                <div v-if="showDropdown" class="dropdown">
                  <ul>
                    <li v-for="alarm in alarms" :key="alarm.id" @click="navigateByType(alarm.type)">
                      {{ alarm.message }} - {{ toLocalTimeStamp(alarm.time).substr(0, 10) }}
                      <button @click.stop="deleteAlarm(alarm.eventId)">확인</button>
                    </li>
                    <li v-if="alarms.length === 0">          
                      알림이 없습니다.
                    </li>
                  </ul>
                </div>
